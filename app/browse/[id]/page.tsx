...
export default async function AssetDetailPage({ params }: Props) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
      <Link
        href="/browse"
        className="inline-flex items-center gap-1.5 text-sm text-[#6B7280] hover:text-[#0057B8] mb-6 transition-colors"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-10">
        ...
        <div className="lg:col-span-3">
          <div className="rounded-2xl overflow-hidden border border-[#D0DBE8] bg-[#F0F4F8]">
            <Image
              src={previewUrl}
              alt={asset.title}
              width={900}
        ...
        <div className="lg:col-span-2 flex flex-col gap-5">
          <div>
            <span className="text-xs font-medium uppercase tracking-widest text-[#0057B8]">
              {asset.type || "photo"}
            </span>
            <h1 className="text-2xl font-bold text-[#003366] mt-1">
              {asset.title}
            </h1>
            <p className="text-[#6B7280] mt-2 text-sm leading-relaxed">{asset.description}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {createdAt && (
              <div className="flex items-center gap-2 text-[#1A1A1A]">
                <Calendar size={14} className="text-[#0057B8]" />
            ...
            <div className="flex items-center gap-2 text-[#1A1A1A]">
              <Download size={14} className="text-[#0057B8]" />
              {asset.downloads || 0} downloads
            </div>
            <div className="flex items-center gap-2 text-[#1A1A1A] capitalize">
              <Tag size={14} className="text-[#0057B8]" />
              {license} license
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {tags.map((tag: string) => (
                <Link
                  key={tag}
                  href={`/browse?q=${tag}`}
                  className="text-xs px-2.5 py-1 bg-[#F0F4F8] text-[#0057B8] rounded-full hover:bg-[#D0DBE8] transition-colors"
                >
                  {tag}
                </Link>
              ))}
          ...
          <Link
            ...
            className="flex items-center gap-3 p-4 bg-white rounded-xl border border-[#D0DBE8] hover:border-[#0057B8] transition-colors"
          >
            <div className="w-10 h-10 rounded-full bg-[#D0DBE8] flex items-center justify-center text-sm font-medium text-[#003366] shrink-0">
              {contributorName.split(" ").map((n: string) => n[0]).join("")}
            </div>
            <div>
              <p className="text-sm font-medium text-[#1A1A1A]">{contributorName}</p>
              <p className="text-xs text-[#6B7280]">View profile →</p>
            </div>
          </Link>

          {/* Free download */}
          <div className="mt-auto pt-4 border-t border-[#D0DBE8]">
            <div className="flex items-baseline gap-2 mb-4">
              <span className="text-3xl font-bold text-[#003366]">
                Free
              </span>
              <span className="text-sm text-[#6B7280] capitalize">{license} license</span>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            {related.map((a: any) => (
              <Link
                key={a.id}
                href={`/browse/${a.id}`}
                className="group rounded-xl overflow-hidden border border-[#D0DBE8] bg-white hover:shadow-md transition-shadow"
              >
                <div className="relative h-36 bg-[#F0F4F8]">
                  <Image
                ...
                <p className="text-xs font-medium text-[#1A1A1A] truncate p-2">{a.title}</p>
              </Link>
            ))}
          </div>
        </div>
